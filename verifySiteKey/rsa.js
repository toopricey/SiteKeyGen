...
/**
 * Reads ASN.1 data matching the template passed in. This will return an
...
function readASN1(data, templ)
{
  ...
  function readNode(curTempl)
  {
    ...
    if ("children" in curTempl)
    {
      ...
      if (pos > end)
        return "Children too large";
    }
    else if ("encapsulates" in curTempl)
    {
      if (next() != 0)
        return "Encapsulation expected";
...
function printDigest(key, signature)
{
  var keyData = readPublicKey(key);
  if (!keyData)
    return "Key fucked up: " + keyData;
...
function printExpected(data)
  {
    ...
    {
      return "Invalid encrypted signature: " + e;
    }
  }
...
function verifySignature(key, signature, data)
{
  ...
  try
  {
    ...
    if (next() != 1)
      return "Wrong padding in signature digest";
    while (next() == 255) {}
